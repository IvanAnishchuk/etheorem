...
describes — the LeanPoseidon library's roadmap. Each stage has a goal, the
...
- **`LeanPoseidonProofs` is standalone, not in the umbrella `[[require]]`**
  ...
  root build and every other gate stay mathlib-free — strengthening, not
  weakening, the isolation goal. (ARCHITECTURE.md §11.)
