...
Stages 5–6 ship the proof *infrastructure* (`@[ssz_simp]` set,
`Supported` predicate, `BasicSupported` dispatch) plus a narrow
first cut; Phase 3 validates the implementation empirically against
the official `ethereum/consensus-spec-tests` release vectors;
Phase 4 builds the performance layer on top of the now-known-good
library; Phase 5 (Stage 18) closes the three theorems
universally over `Supported`. Until Stage 18 lands, the
`SSZ.roundtrip` user-surface corollary is gated by
`BasicSupported r.shape`. The asterisk on "verified by inheritance"
is intentional and small: passing empirical conformance is what
...
| **5 — Complete formal verification** | **Stage 18 — widen `BasicSupported` to all of `SSZType.Supported` / `SupportedBounded`, closing `decode_encode`, `serialize_injective`, `encode_size_le_max` universally.** | Positioned last by design: empirical conformance from Phase 3 ensures the proof effort targets a known-correct implementation, not a speculative one. The publishable non-malleability artefact lands here. |
