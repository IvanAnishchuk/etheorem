...
A planned `@[csimp]`-attributed equality between the FFI and the spec
hashers would replace these axioms with theorems carrying the same
statements; every dependent proof tightens automatically — no theorem
