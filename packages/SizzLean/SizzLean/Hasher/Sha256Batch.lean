...
validated by `Sha256BatchEquivalence.lean`; promoted here to a named
Lean axiom so proofs that depend on the batched primitive can be
audited via `#axioms`. Same trust footprint as `sha256Combine_eq_spec`
— we trust OpenSSL plus the C shim's loop. Replaceable by a
`@[csimp]`-proved theorem when one lands; proof shapes stay
identical. -/
